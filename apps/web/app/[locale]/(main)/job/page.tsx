...
export default async function Home({ params }: PageProps<"/[locale]/job">) {
  const locale = await setCurrentLocaleFromParams(params);

  return (
    <>
      <div>
        <section className="py-20 min-h-[calc(100lvh-(var(--spacing-header)))] flex justify-center flex-col">
          {locale === "ja" ? (
