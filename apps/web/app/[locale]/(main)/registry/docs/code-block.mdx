...
```tsx
const { currentId, setCurrentId, codes, groups, currentGroup } = useCodeBlock();
```
...
```tsx
import { CopyCodeButton } from "@/components/code-block";

// CopyCodeButtonコンポーネントは自動的にコンテキストから
// 現在のコードを取得します
<CopyCodeButton />
```
...
- `CodeBlockGroupProvider` は、グループ機能を使用する場合に必須です
